...
function reverseCharacters(input) {
    if (typeof(input) === 'string') {
        ...
        // if the input is a number - turns number into string to reverse then back into a number for output 
        input = String(input);
        let numArr = input.split('').reverse().join('');
...
function completeReversal(inputArr) {
    let reversedArr = [];
    for (let i = 0; i < inputArr.length; i++) {
        reversedArr.push(reverseCharacters(inputArr[i]));
    }
